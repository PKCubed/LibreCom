...
static void decode_uart_frames(int16_t *decoded_pcm) {
    static enum {
        RX_WAIT_SYNC0,
        RX_WAIT_SYNC1,
        RX_WAIT_LEN0,
    ...
    static uint8_t payload[PCM_FRAME_BYTES];
    static uint16_t expected_len = 0;
    static uint16_t pos = 0;
    static uint8_t len_lo = 0;
    ...
    while (uart_ring_pop(&b)) {
        switch (state) {
            ...
            case RX_WAIT_LEN1:
                expected_len = (uint16_t)len_lo | ((uint16_t)b << 8);
                pos = 0;
                if (expected_len == PCM_FRAME_BYTES && expected_len <= sizeof(payload)) {
            ...
            case RX_WAIT_CRC:
                if (checksum_bytes(payload, expected_len) == b) {
                    for (uint16_t i = 0; i < PCM_FRAME_SAMPLES; ++i) {
                        const uint16_t sample_bits = (uint16_t)payload[(i * 2) + 0] | ((uint16_t)payload[(i * 2) + 1] << 8);
                        int16_t sample = (int16_t)sample_bits;
                        (void)pcm_ring_push(sample);
                    }
                }
                state = RX_WAIT_SYNC0;
                break;
...
int main(void) {
    ...
    int16_t *decoded_pcm = (int16_t *)calloc(PCM_FRAME_SAMPLES, sizeof(int16_t));
    if (!decoded_pcm) {
        while (true) {
            tight_loop_contents();
        }
    ...
    while (true) {
        decode_uart_frames(decoded_pcm);
